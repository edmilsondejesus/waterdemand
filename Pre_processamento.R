...
#--------------------   Limpeza: Identifica��o e substitui��o de dados inv�lidos ----------
...
#Finalizado o pr�processamento dos dados de consumo


#----------------------------------------------------------------------------------
#----------------------------------------------------------------------------------
#-----------------------------------Tratamento dos dados de metereol�gicos ------------------------------------------
...
#carrega o dia do m�s baseado na data
...
#Converte temperatura para formato num�rico
...
#Converte velocidade para formato num�rico
...
#Converte press�o para caracter num�rico
dados_clima$PRESSAO_ATMOSFERICA <- as.numeric(dados_clima$PRESSAO_ATMOSFERICA)

#Converte precipitacao para caracter num�rico
...
# Gr�fico de umidade relativa 24/12/2019
...
#Correla��o umidade relativa x temperatura 24/12/2020
...
# Histograma da vari�vel temperatura
...
#Rela��o entre temperatura x umdiade relativa
plot(y=dados_clima2$TEMPERATURA_DO_AR_C, x = dados_clima2$UMIDADE_RELATIVA_DO_AR, main = 'Rela��o entre a Temperatura x Umidade Relativa')

rm(dados_clima2)

#------------------------------------------------------------
#-> fazer um levantamento em cada vari�vel para identificar at� quando tem dados ausentes e qual a frequencia.
...
# dados inv�lidos de temperatura por ano
ggplot(df_dados_aus, aes(x=NN_ANO, y=TOTAL, fill=NN_ANO))+ 
  geom_bar(stat = "identity", color ="black", fill="red", width=0.30) +
  ggtitle("Quantidade de dados inv�lidos por ano") +
  xlab("Ano") + ylab("Qtd de erros")
rm(df_dados_aus)

# dados inv�lidos por vari�vel e ano
df_erros_ano <-sqldf::sqldf("select sum(CASE WHEN VELOCIDADE_VENTO < -100 THEN 1 ELSE 0 end)'Velocidade vento',sum(CASE WHEN PRESSAO_ATMOSFERICA < -100 THEN 1 ELSE 0 end)'Press�o atmosf�rica', sum(CASE WHEN PRECIPITACAO < -100 THEN 1 ELSE 0 end)'Precipita��o', NN_ANO, sum(CASE WHEN TEMPERATURA_DO_AR_C < -100 THEN 1 ELSE 0 end)'Temperatura',sum(CASE WHEN UMIDADE_RELATIVA_DO_AR < -100 THEN 1 ELSE 0 end)'Umidade relativa'   from dados_clima  where NN_ANO > 2016 group by NN_ANO")
...
ggplot(df_erros_ano.long,aes(x=NN_ANO,y=value,fill=factor(variable)))+
  ...
  scale_fill_discrete(name="Vari�vel")+
  ylab("Quantidade de erros")+xlab('Ano')+
  theme(plot.title = element_text(hjust = 0.5))+
  ggtitle('Dados metereol�gicos inv�lidos por ano')+
...
# dados metereol�gicos por vari�vel e ano
df_erros_ano <-sqldf::sqldf("select sum(1)'Velocidade vento',sum(1)'Press�o atmosf�rica', sum(1)'Precipita��o', NN_ANO, sum(1)'Temperatura',sum(1)'Umidade relativa'   from dados_clima  where NN_ANO > 2016 group by NN_ANO")
df_erros_ano.long<-melt(df_erros_ano,id.vars="NN_ANO") # formatar para long
ggplot(df_erros_ano.long,aes(x=NN_ANO,y=value,fill=factor(variable)))+
  geom_bar(stat="identity",position="dodge")+
  scale_fill_discrete(name="Vari�vel")+
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  ggtitle('Dados metereol�gicos por ano')+
  geom_text(aes(label=value), position=position_dodge(width=0.9), vjust=-0.25, size=1.5)
rm(df_erros_ano.long)
rm(df_erros_ano)
...
#repete o gr�fico sem os dados de 2016
# dados inv�lidos por vari�vel e ano
df_erros_ano <-sqldf::sqldf("select sum(CASE WHEN VELOCIDADE_VENTO < -100 THEN 1 ELSE 0 end)'Velocidade vento',sum(CASE WHEN PRESSAO_ATMOSFERICA < -100 THEN 1 ELSE 0 end)'Press�o atmosf�rica', sum(CASE WHEN PRECIPITACAO < -100 THEN 1 ELSE 0 end)'Precipita��o', NN_ANO, sum(CASE WHEN TEMPERATURA_DO_AR_C < -100 THEN 1 ELSE 0 end)'Temperatura',sum(CASE WHEN UMIDADE_RELATIVA_DO_AR < -100 THEN 1 ELSE 0 end)'Umidade relativa'   from datasetest1  group by NN_ANO")
...
ggplot(df_erros_ano.long,aes(x=NN_ANO,y=value,fill=factor(variable)))+
  ...
  ylab("Quantidade de erros")+xlab('Ano')+
  theme(plot.title = element_text(hjust = 0.5))+
  ggtitle('Dados inv�lidos por ano')+
  geom_text(aes(label=value), position=position_dodge(width=0.9), vjust=-0.25, size=1.5)
...
#substitui todos os dados ausentes pela m�dia das 24h
...
df_erros_ano <-sqldf::sqldf("select sum(CASE WHEN VELOCIDADE_VENTO < -100 THEN 1 ELSE 0 end)'Velocidade vento',sum(CASE WHEN PRESSAO_ATMOSFERICA < -100 THEN 1 ELSE 0 end)'Press�o atmosf�rica', sum(CASE WHEN PRECIPITACAO < -100 THEN 1 ELSE 0 end)'Precipita��o', NN_ANO, sum(CASE WHEN TEMPERATURA_DO_AR_C < -100 THEN 1 ELSE 0 end)'Temperatura',sum(CASE WHEN UMIDADE_RELATIVA_DO_AR < -100 THEN 1 ELSE 0 end)'Umidade relativa'   from ds_clima_tratado  group by NN_ANO")
df_erros_ano.long<-melt(df_erros_ano,id.vars="NN_ANO") # formatar para long
ggplot(df_erros_ano.long,aes(x=NN_ANO,y=value,fill=factor(variable)))+
  geom_bar(stat="identity",position="dodge")+
  scale_fill_discrete(name="Vari�vel")+
...
#Merge dos dados metereol�gicos e de consumo
...
View(ds_data)
#Merge dos dados metereol�gicos e de consumo unificado
